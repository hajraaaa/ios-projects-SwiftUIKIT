...
struct Country {
    ...
    // Grouping countries by the first letter
        static func getCountriesGroupedByAlphabet() -> [String: [Country]] {
            let countries = allCountries.map { Country(name: $0) }
            let groupedCountries = Dictionary(grouping: countries) { String($0.name.prefix(1)) }
            return groupedCountries
        }
        
        // Search function to filter countries by search text
        static func searchCountries(by searchText: String) -> [Country] {
            return allCountries
